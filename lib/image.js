 ...
 * @constructor
 */
function Generator(file, list, settings) {
...
Generator.prototype = {
    ...
    after: function (image, arr_selector, direct, scale) {
        ...
        // 生成合成图片文件名
        var fileNameReg = new RegExp(this.settings.workbench + '/build/css/' + '(.+)\\.[^.]+$'),
            fileName = fileNameReg.exec(this.file.path)[1];

        fileName = fileName.replace(/\//g, '-').replace('.', '-');
        fileName += ext;

        var imageLocalDir = this.settings.workbench + '/build/image/sprite/',
            imageLocalUrl = imageLocalDir + fileName;

        // 检测目录是否存在
        if (!fs.existsSync(imageLocalDir)) {
            fs.mkdirSync(imageLocalDir);
        }
        fs.writeFileSync(imageLocalUrl, image.encode('png'));

        // 加上 md5 戳
        var md5 = crypto.createHash('md5').update(new Date().getTime() + '').digest('hex').substring(0, 10),
            imageUrl = this.settings.imagePre + '/image/sprite/' + fileName + '?v=' + md5;
...
module.exports = function (file, list, settings) {
    var gen = new Generator(file, list, settings);
    return gen.css;
};
